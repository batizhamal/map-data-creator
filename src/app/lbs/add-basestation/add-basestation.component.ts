...
export class IconImage {
	constructor(public readonly url: string, public readonly name: string, public loaded = false) {}
...
export class AddBasestationComponent implements OnInit {
	...
	private readonly iconsUrls: ReadonlySet<string> = new Set(['/assets/icons/basestation.png']);
	...
	addBasestationPoint({ lngLat }) {
		if (lngLat) {
			...
			this.featureCollectionPoints.push(turf.point([lng, lat]));

			const coordsHashId = `${lat}-${lng}`;
			if (!this.basestationPointsHashes.has(coordsHashId)) {
				const point = turf.point([lng, lat]);
				this.basestationPointsHashes.set(coordsHashId, point);
			}
			this.basestationPoints = turf.featureCollection(Array.from(this.basestationPointsHashes.values()));
  ...
  deleteBasestations() {
    this.basestationPointsHashes.clear();
    this.basestationPoints = turf.featureCollection(Array.from(this.basestationPointsHashes.values()));
  }
}
